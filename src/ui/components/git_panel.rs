...
pub enum GitPanelHit {
    ...
    CommitInput { rel_x: f64, rel_y: f64 },
    CommitButton,
    GenerateButton,
    StageAll,
    UnstageAll,
...
impl GitPanelState {
    ...
    pub fn selection_range(&self) -> Option<(usize, usize)> {
        self.selection_anchor.map(|a| {
            ...
            (s.min(self.commit_message.len()), e.min(self.commit_message.len()))
        })
    }
    ...
    pub fn move_down(&mut self, shift: bool, max_chars: usize) {
        ...
        let vlines = wrap_lines(&self.commit_message, max_chars);
        let (row, col) = visual_cursor_pos(&vlines, self.cursor);
        if row + 1 >= vlines.len() { self.cursor = self.commit_message.len(); return; }
...
pub fn content_height(state: &GitPanelState, tab: GitPanelTab, sf: f32) -> f32 {
    match tab {
        ...
        GitPanelTab::Branches => {
            ...
            if local > 0 { h += item_h + local as f32 * item_h; }
            if remote > 0 { h += SECTION_PAD_Y * sf + item_h + remote as f32 * item_h; }
            h
        }
    }
}
        ...
        if logical_line.is_empty() {
            ...
            while start < logical_line.len() {
                ...
                let end = if end < logical_line.len() {
                    logical_line[..end].char_indices().next_back()
                        .map(|(i, c)| i + c.len_utf8()).unwrap_or(end)
                } else {
                    end
                };
...
fn changes_content_height(state: &GitPanelState, sf: f32) -> f32 {
    ...
    if staged > 0 { h += section_h + staged as f32 * item_h + SECTION_PAD_Y * sf; }
    if unstaged > 0 { h += section_h + unstaged as f32 * item_h; }
    if staged == 0 && unstaged == 0 { h += 60.0 * sf; }
    h
}
...
pub fn update_hover(
    ...
    if state.hovered_toolbar_btn != new_toolbar { state.hovered_toolbar_btn = new_toolbar; changed = true; }

    let new_commit = matches!(hit, GitPanelHit::CommitButton);
    if state.hovered_commit_btn != new_commit { state.hovered_commit_btn = new_commit; changed = true; }

    let new_gen = matches!(hit, GitPanelHit::GenerateButton);
    if state.hovered_generate_btn != new_gen { state.hovered_generate_btn = new_gen; changed = true; }

    let new_stage_all = matches!(hit, GitPanelHit::StageAll);
    if state.hovered_stage_all != new_stage_all { state.hovered_stage_all = new_stage_all; changed = true; }

    let new_unstage_all = matches!(hit, GitPanelHit::UnstageAll);
    if state.hovered_unstage_all != new_unstage_all { state.hovered_unstage_all = new_unstage_all; changed = true; }

    let new_item = match &hit {
        GitPanelHit::SelectFile(idx) | GitPanelHit::StageFile(idx) | GitPanelHit::UnstageFile(idx) => Some(*idx),
        _ => None,
    };
    if state.hovered_item != new_item { state.hovered_item = new_item; changed = true; }

    let new_branch = match &hit {
        GitPanelHit::CheckoutBranch(idx) => Some(*idx),
        _ => None,
    };
    if state.hovered_branch != new_branch { state.hovered_branch = new_branch; changed = true; }
...
pub fn draw(
    ...
    if !state.data.has_repo {
        draw_no_repo(buf, font_system, swash_cache, panel_x, bar_h, panel_w, panel_h, sf);
        return panel_w;
    }

    let header_h = (HEADER_HEIGHT * sf) as usize;
    draw_toolbar(buf, icon_renderer, state, panel_layout, panel_x, bar_h, panel_w, header_h, sf);
    ...
    match panel_layout.git_tab {
        GitPanelTab::Changes => {
            draw_changes_tab(buf, font_system, swash_cache, icon_renderer, state, panel_x, content_y, panel_w, content_h, sf, scroll, cursor_blink_on);
        }
        GitPanelTab::Branches => {
            draw_branches_tab(buf, font_system, swash_cache, icon_renderer, state, panel_x, content_y, panel_w, content_h, sf, scroll);
        }
    }
    ...
    draw_git_scrollbar(buf, panel_x, panel_w, content_y, content_h, total_h, scroll, sb_hover, sf);
...
fn draw_toolbar(
    ...
    for (i, (tab, icon)) in tabs.iter().enumerate() {
        ...
        let color = if is_active { theme::FG_BRIGHT } else if is_hovered { theme::FG_PRIMARY } else { theme::FG_MUTED };
...
fn draw_no_repo(
    buf: &mut PixelBuffer, font_system: &mut FontSystem, swash_cache: &mut SwashCache,
    panel_x: usize, bar_h: usize, panel_w: usize, panel_h: usize, sf: f32,
) {
    ...
    draw_text_at(buf, font_system, swash_cache, tx, ty, clip_h, msg, metrics, theme::FG_MUTED, Family::SansSerif);
}

fn draw_changes_tab(
    buf: &mut PixelBuffer, font_system: &mut FontSystem, swash_cache: &mut SwashCache,
    icon_renderer: &mut IconRenderer, state: &GitPanelState,
    panel_x: usize, content_y: usize, panel_w: usize, content_h: usize, sf: f32,
    scroll: usize, cursor_blink_on: bool,
) {
    ...
    if y >= content_y && y + input_h <= max_y {
        let border_color = if state.commit_input_focused { theme::PRIMARY } else { theme::BORDER };
        ...
        if state.generating_commit_msg && state.commit_message.is_empty() {
            ...
            draw_text_at(buf, font_system, swash_cache, text_x, text_y, clip_h, "Generating…", text_metrics, theme::FG_MUTED, Family::SansSerif);
        } else if state.commit_message.is_empty() {
            let text_y = y + (min_input_h as f32 / 2.0 - 7.0 * sf) as usize;
            let placeholder = format!("Message (\u{2318}Enter on \"{}\")", state.data.current_branch);
            let truncated = truncate_to_fit(&placeholder, char_w, text_max_px);
            draw_text_at(buf, font_system, swash_cache, text_x, text_y, clip_h, &truncated, text_metrics, theme::FG_MUTED, Family::SansSerif);
            ...
            for (vline_start, vline_text) in &visual_lines {
                ...
                if let Some((sel_s, sel_e)) = sel {
                    if sel_s < vline_end && sel_e > *vline_start {
                        ...
                        buf.fill_rect(hl_x, ly, hl_w.min(input_w - (8.0 * sf) as usize * 2), line_h, (60, 90, 160));
                    }
                }
                draw_text_at(buf, font_system, swash_cache, text_x, ly, clip_h, vline_text, text_metrics, (220, 220, 220), Family::SansSerif);
                if state.commit_input_focused && cursor_blink_on
                    && state.cursor >= *vline_start && state.cursor <= vline_end
                {
    ...
    if y >= content_y && y + btn_h <= max_y {
        ...
        let btn_color = if state.hovered_commit_btn { (235, 50, 135) } else { theme::PRIMARY };
        ...
        draw_text_at(buf, font_system, swash_cache, label_x, label_y, clip_h, label, label_metrics, (255, 255, 255), Family::SansSerif);
    }
    y += btn_h + pad;

    let staged: Vec<_> = state.data.entries.iter().enumerate().filter(|(_, e)| e.staged).collect();
    let unstaged: Vec<_> = state.data.entries.iter().enumerate().filter(|(_, e)| !e.staged).collect();

    if !staged.is_empty() {
        let sh = (SECTION_HEADER_HEIGHT * sf) as usize;
        if y >= content_y && y + sh <= max_y {
            draw_section_header(buf, font_system, swash_cache, icon_renderer, panel_x, y, panel_w, sf, clip_h,
                "Staged Changes", staged.len(), state.hovered_unstage_all, true);
        }
        y += sh;
        ...
        for (global_idx, entry) in &staged {
            ...
            if y + item_h > content_y {
                ...
                draw_file_entry(buf, font_system, swash_cache, icon_renderer, entry, panel_x, y, panel_w, item_h, sf, hovered, clip_h, true);
            }
            y += item_h;
        }
    ...
    if !unstaged.is_empty() {
        ...
        if y >= content_y && y + sh <= max_y {
            draw_section_header(buf, font_system, swash_cache, icon_renderer, panel_x, y, panel_w, sf, clip_h,
                "Changes", unstaged.len(), state.hovered_stage_all, false);
        }
        y += sh;
        ...
        for (global_idx, entry) in &unstaged {
            ...
            if y + item_h > content_y {
                let hovered = state.hovered_item == Some(*global_idx);
                draw_file_entry(buf, font_system, swash_cache, icon_renderer, entry, panel_x, y, panel_w, item_h, sf, hovered, clip_h, false);
            }
            y += item_h;
        }
    }
    ...
    if staged.is_empty() && unstaged.is_empty() {
        ...
        let tx = panel_x + ((panel_w as f32 - text_w) / 2.0).max(0.0) as usize;
        draw_text_at(buf, font_system, swash_cache, tx, y + (40.0 * sf) as usize, clip_h, msg, metrics, theme::FG_MUTED, Family::SansSerif);
    }
}
...
fn changes_hit_test(content_rel_y: f64, rel_x: f64, panel_w: f64, state: &GitPanelState, sf: f64) -> GitPanelHit {
    ...
    if content_rel_y >= input_y && content_rel_y < input_end {
        ...
        return GitPanelHit::CommitInput { rel_x: click_x, rel_y: click_y };
    }
    ...
    if !staged.is_empty() {
        if content_rel_y >= y && content_rel_y < y + section_h {
            if rel_x >= header_action_zone { return GitPanelHit::UnstageAll; }
            return GitPanelHit::None;
        }
        y += section_h;
    ...
    if !unstaged.is_empty() {
        if content_rel_y >= y && content_rel_y < y + section_h {
            if rel_x >= header_action_zone { return GitPanelHit::StageAll; }
            return GitPanelHit::None;
        }
        y += section_h;
...
fn draw_section_header(
    buf: &mut PixelBuffer, font_system: &mut FontSystem, swash_cache: &mut SwashCache,
    icon_renderer: &mut IconRenderer, panel_x: usize, y: usize, panel_w: usize,
    sf: f32, clip_h: usize, label: &str, count: usize, action_hovered: bool, is_staged: bool,
) {
    ...
    icon_renderer.draw(buf, Icon::ChevronDown, chev_x, chev_y, chev_sz, theme::FG_MUTED);
    ...
    draw_text_at(buf, font_system, swash_cache, label_x, label_y, clip_h, label, label_metrics, theme::FG_PRIMARY, Family::SansSerif);
    ...
    super::overlay::fill_rounded_rect(buf, badge_x, badge_y, badge_sz, badge_sz, badge_r, (55, 55, 62));
    ...
    draw_text_at(buf, font_system, swash_cache, count_x, count_y, clip_h, &count_str, count_metrics, (200, 200, 200), Family::Monospace);
    ...
    let action_color = if action_hovered { theme::FG_BRIGHT } else { theme::FG_MUTED };
    icon_renderer.draw(buf, action_icon, action_x, action_y, action_sz, action_color);
}

fn draw_file_entry(
    buf: &mut PixelBuffer, font_system: &mut FontSystem, swash_cache: &mut SwashCache,
    icon_renderer: &mut IconRenderer, entry: &StatusEntry,
    panel_x: usize, y: usize, panel_w: usize, item_h: usize, sf: f32, hovered: bool, clip_h: usize,
    is_staged: bool,
) {
    if hovered {
    ...
    let text_color = if hovered { theme::TAB_ACTIVE_TEXT } else { (200, 200, 200) };
    ...
    if parent.is_empty() {
        ...
        draw_text_at(buf, font_system, swash_cache, text_x, text_y, clip_h, &truncated, metrics, text_color, Family::SansSerif);
    } else {
        let name_max = text_max_px * 0.6;
        let truncated_name = truncate_to_fit(basename, char_w, name_max);
        draw_text_at(buf, font_system, swash_cache, text_x, text_y, clip_h, &truncated_name, metrics, text_color, Family::SansSerif);
        ...
        if dir_max > 10.0 * sf {
            ...
            draw_text_at(buf, font_system, swash_cache, dir_x, text_y, clip_h, &truncated_dir, dir_metrics, theme::FG_MUTED, Family::SansSerif);
        }
    }
    ...
    draw_text_at(buf, font_system, swash_cache, badge_x, badge_y, clip_h, badge_char, badge_metrics, badge_color, Family::Monospace);

    if hovered {
        let action_x = badge_x.saturating_sub(action_sz as usize + (6.0 * sf) as usize);
        let action_y = y + (item_h - action_sz as usize) / 2;
        let action_icon = if is_staged { Icon::Close } else { Icon::Plus };
        icon_renderer.draw(buf, action_icon, action_x, action_y, action_sz, theme::FG_BRIGHT);
    }
}

fn draw_rounded_border(
    buf: &mut PixelBuffer, x: usize, y: usize, w: usize, h: usize, color: Rgb, sf: f32,
) {
...
fn draw_branches_tab(
    ...
    if state.data.branches.is_empty() {
        ...
        if ty >= content_y {
            draw_text_at(buf, font_system, swash_cache, panel_x + pad_x, ty, clip_h, msg, metrics, theme::FG_MUTED, Family::SansSerif);
        }
        return;
    }
    ...
    let local: Vec<_> = state.data.branches.iter().filter(|b| !b.is_remote).collect();
    ...
    if !local.is_empty() {
        ...
        if y >= content_y && y + item_h <= max_y {
            draw_text_at(buf, font_system, swash_cache, panel_x + pad_x, y, clip_h,
                &format!("Local ({})", local.len()), label_metrics, theme::FG_MUTED, Family::SansSerif);
        }
        y += item_h;
        for branch in &local {
            if y >= max_y { break; }
            if y + item_h > content_y {
                if branch.is_current { buf.fill_rect(panel_x, y, panel_w, item_h, theme::BG_ELEVATED); }
                let icon_sz = (14.0 * sf).round() as u32;
                let icon_x = panel_x + pad_x;
                let icon_y = y + (item_h - icon_sz as usize) / 2;
                let icon_color = if branch.is_current { theme::PRIMARY } else { theme::FG_MUTED };
                icon_renderer.draw(buf, Icon::GitBranch, icon_x, icon_y, icon_sz, icon_color);
                let text_x = icon_x + icon_sz as usize + (6.0 * sf) as usize;
                let text_y = y + (item_h as f32 / 2.0 - 8.0 * sf) as usize;
                let text_color = if branch.is_current { (255, 255, 255) } else { (200, 200, 200) };
                let branch_max = max_text_px - icon_sz as f32 - 6.0 * sf;
                let display = truncate_to_fit(&branch.name, char_w, branch_max);
                draw_text_at(buf, font_system, swash_cache, text_x, text_y, clip_h, &display, metrics, text_color, Family::SansSerif);
            }
            y += item_h;
        }
    }
    ...
    if !remote.is_empty() {
        ...
        if y >= content_y && y + item_h <= max_y {
            draw_text_at(buf, font_system, swash_cache, panel_x + pad_x, y, clip_h,
                &format!("Remote ({})", remote.len()), label_metrics, theme::FG_MUTED, Family::SansSerif);
        }
        y += item_h;
        for branch in &remote {
            ...
            if y + item_h > content_y {
                ...
                draw_text_at(buf, font_system, swash_cache, panel_x + pad_x, text_y, clip_h, &display, metrics, theme::FG_MUTED, Family::SansSerif);
            }
            y += item_h;
        }
    }
...
fn branches_hit_test(content_rel_y: f64, state: &GitPanelState, sf: f64) -> GitPanelHit {
    let item_h = FILE_ITEM_HEIGHT as f64 * sf;
    let section_pad = SECTION_PAD_Y as f64 * sf;
    let local: Vec<_> = state.data.branches.iter().enumerate().filter(|(_, b)| !b.is_remote).collect();
...
pub fn git_scrollbar_hit_test(
    px: usize, py: usize,
    panel_x: usize, panel_w: usize, y_start: usize, visible_h: usize,
    total_h: usize, scroll: usize, sf: f32,
) -> bool {
    if let Some((tx, ty, tw, th)) = git_scrollbar_thumb_rect(panel_x, panel_w, y_start, visible_h, total_h, scroll, sf) {
...
mod tests {
    ...
    fn staged_unstaged_split() {
        ...
        s.data.entries = vec![
            StatusEntry { path: "a.rs".into(), status: FileStatus::Modified, staged: true },
            StatusEntry { path: "b.rs".into(), status: FileStatus::Added, staged: false },
            StatusEntry { path: "c.rs".into(), status: FileStatus::Deleted, staged: true },
        ];
    ...
    fn status_colors_unique() {
        let colors = [
            status_color(FileStatus::Modified), status_color(FileStatus::Added),
            status_color(FileStatus::Deleted), status_color(FileStatus::Renamed),
            status_color(FileStatus::Untracked), status_color(FileStatus::Conflicted),
        ];
    ...
    fn toolbar_hit_detects_tabs() {
        ...
        let x = pad + 5.0;
        assert_eq!(toolbar_hit_test(x, header_h, sf), GitPanelHit::ToolbarChanges);
    }
